...
public class GoldSheep extends Animal implements IAnimatable, Shearable, net.minecraftforge.common.IForgeShearable {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.model.gold_sheep", true));
    ...
    public static void cowDropWool(LevelAccessor world, double x, double y, double z, Entity entity,ItemStack itemstack) {
        ...
        if (world instanceof Level _level && !_level.isClientSide()) {
            ItemEntity entityToSpawn = new ItemEntity(_level, x, y, z, new ItemStack(Items.GOLD_NUGGET,12));
    ...
    public GoldSheep getBreedOffspring(ServerLevel serverLevel, AgeableMob ageableMob) {
        ...
        if (ageableMob instanceof GoldSheep2) {
            woolCow = ModEntityTypesWD.GOLD_SHEEP.get().create(serverLevel);
        }
        else if (ageableMob instanceof GoldSheep) {
            woolCow = ModEntityTypesWD.GOLD_SHEEP.get().create(serverLevel);
        }else {
            woolCow = (GoldSheep) ageableMob;
            GoldSheep GoldSheep;
